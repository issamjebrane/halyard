...
// Visibility into the automated pipeline: Telegram -> Supabase (ingest) and
// Supabase -> MT5 (executions). Pure presentational; data is gathered server-side.
    ...
    <section className="space-y-3">
      <h2 className="flex items-center gap-2 font-mono text-xs uppercase tracking-wider text-muted">
        ...
        <InfoTip label="What operations shows">
          The automated pipeline. <span className="text-foreground">ingest</span> = signals
          read from the Telegram channel into the database.{" "}
          <span className="text-foreground">executions</span> = what the MT5 copier actually
          did with them (placed / skipped / error). The trust score never reads executions.
        </InfoTip>
      </h2>
